...
@TeleOp(name="OMEGA Tele Op", group="Linear OpMode")

public class omegaTeleOp extends LinearOpMode {
    private double turretAngle;
    private double turretPos;
    private final double TURRET_MAX = 135;
    private final double TURRET_MIN = -135;

    private final double INTAKE_MAX = .6;
    private double intakePos = 0;
    private double tPower = 0;
    private final double INTKAE_MIN =.6;
    private final double OUTTAKE_MAX  = .2;
    private final double OUTTAKE_MIN = .8;
    ...
    private DcMotorEx frontLeftDrive, backLeftDrive, frontRightDrive, backRightDrive, outtake1, outtake2, transfer1,transfer2;
    private Servo t1, t2, t3,hood,i1,i2;
    ...
    private void teleInit() {
        ...
        transfer1 = hardwareMap.get(DcMotorEx.class, HConst.TRANSFER1);
        transfer2 = hardwareMap.get(DcMotorEx.class,HConst.TRANSFER2);
        //Servos
        t1=hardwareMap.get(Servo.class,HConst.TURRET1);
        t2=hardwareMap.get(Servo.class,HConst.TURRET2);
        t3=hardwareMap.get(Servo.class,HConst.TURRET3);
        hood=hardwareMap.get(Servo.class,HConst.HOOD);
        i1=hardwareMap.get(Servo.class,HConst.INTAKE_LEFT_SERVO);
        i2=hardwareMap.get(Servo.class,HConst.INTAKE_RIGHT_SERVO);
        //shooter system
        ...
        t1.setDirection(HConst.TURRET_DIR);
        t2.setDirection(HConst.TURRET_DIR);
        t3.setDirection(HConst.TURRET_DIR);
        hood.setDirection(HConst.HOOD_DIR);
        i1.setDirection(HConst.INTAKE1_SERV_DIR);
        i2.setDirection(HConst.INTAKE2_SERV_DIR);
    ...
    private void updateAuxiliaryMotors() {
       if (driveTrainMode == HConst.DriveTrainMode.AUTO_TARGET_GOAL) {
            ...
            double turretOffset = p * error;

            if (Math.abs(error) > 0.5)
                turretOffset += Math.signum(error) * kStatic;
            turretOffset = Math.max(-0.4, Math.min(0.4, turretOffset));
            ...
            turn(turretPos-turretOffset);
      }
        double transferPower = .2;
        if (allOn)
            transferPower = 1;
        if(intaking)
            transferPower = 0.5;
        ...
        i1.setPosition(intakePos);
        i2.setPosition(intakePos);
 ...
 */
        telemetry.addData("INTAKE POwer: ",tPower);
        telemetry.addData("TRANSFER POWER: ", transfer1.getPower());
    ...
    private void turn(double position){
        position = Math.max(0.0, Math.min(1.0, position));
        turretPos = position;
        t1.setPosition(position);
        t2.setPosition(position);
        t3.setPosition(position);
        turretAngle= getAngleFromPosition(position);
    }
    private double getAngleFromPosition(double pos){
        return (270*pos)-135;
    }
